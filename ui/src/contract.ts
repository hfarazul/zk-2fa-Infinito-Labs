...
export async function verifyAndSend(input: Object, amount: string, recepient: string) {
    ...
    if (calldata) {
        ...
        // console.log(`here`)
        // console.log(tx)
        tx = await otp.verifyAndSend(calldata[0], calldata[1], calldata[2], calldata[3], recepient, {value: ethers.utils.parseEther(amount)})
            .catch((error: any) => {
                console.log(error);
                let errorMsg;
                if (error.reason) {
